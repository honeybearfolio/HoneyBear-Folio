...
pub fn read_xlsx(data: Vec<u8>) -> Result<ReadXlsxResult, String> {
    ...
    let mut workbook: Xlsx<_> = open_workbook_from_rs(cursor).map_err(|e: calamine::XlsxError| e.to_string())?;
        ...
        for cell in row {
            let val = match cell {
                ...
                Data::Float(f) => {
                     ...
                     serde_json::Number::from_f64(f.to_owned()).map(Value::Number).unwrap_or(Value::Null)
                },
                Data::String(s) => Value::String(s.clone()),
                Data::Bool(b) => Value::Bool(b.to_owned()),
                Data::DateTime(d) => Value::Number(serde_json::Number::from_f64(d.as_f64().to_owned()).unwrap_or(serde_json::Number::from(0))), // Excel serial date
...
pub fn write_xlsx(file_path: String, sheets: Vec<SheetData>) -> Result<(), String> {
    ...
    for sheet_data in sheets {
        ...
        worksheet.set_name(&sheet_data.name).map_err(|e| e.to_string())?;
        ...
        let headers: Vec<String> = if let Value::Object(map) = first_row {
            ...
            return Err(format!("Data in sheet '{}' is not an array of objects", sheet_data.name));
        };
                ...
                for (col, header) in headers.iter().enumerate() {
                    if let Some(val) = map.get(header) {
                         match val {
                            ...
                            Value::Bool(b) => {
                                worksheet.write_boolean((row_idx + 1) as u32, col as u16, *b).map_err(|e| e.to_string())?;
                            },
                            Value::Number(n) => {
                                if let Some(f) = n.as_f64() {
                                    worksheet.write_number((row_idx + 1) as u32, col as u16, f).map_err(|e| e.to_string())?;
                                }
                            },
                            Value::String(s) => {
                                worksheet.write_string((row_idx + 1) as u32, col as u16, s).map_err(|e| e.to_string())?;
                            },
                            Value::Array(_) | Value::Object(_) => {
                                worksheet.write_string((row_idx + 1) as u32, col as u16, &val.to_string()).map_err(|e| e.to_string())?;
                            }
                         }
                    }
                }
