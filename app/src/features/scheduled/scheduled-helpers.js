...
export function toScheduledPayload(formState, translate) {
  ...
  if (isInvestment) {
    ...
    amount = formState.isBuy ? -(sharesNum * priceNum + feeNum) : sharesNum * priceNum - feeNum;
  }

  return {
    accountId: formState.accountId,
    payee: isInvestment
      ? formState.payee.trim() || (formState.isBuy ? translate("scheduled.field.buy") : translate("scheduled.field.sell"))
      : formState.payee.trim(),
    amount,
    category: isInvestment
      ? formState.category.trim() || translate("scheduled.field.investment_category")
      : formState.category.trim() || null,
    notes: formState.notes.trim() || null,
    currency: formState.currency || null,
    recurrenceType: formState.recurrenceType,
    intervalValue: formState.recurrenceType === "every_n" ? Number(formState.intervalValue) || 1 : null,
    intervalUnit: formState.recurrenceType === "every_n" ? formState.intervalUnit : null,
    daysOfWeek: formState.recurrenceType === "day_of_week" ? formState.daysOfWeek : null,
    ordinal: formState.recurrenceType === "ordinal_weekday" ? Number(formState.ordinal) : null,
    weekday: formState.recurrenceType === "ordinal_weekday" ? Number(formState.weekday) : null,
    startDate: formState.startDate,
    endDate: formState.endDate || null,
    maxOccurrences: formState.maxOccurrences ? Number(formState.maxOccurrences) : null,
    transactionType: formState.transactionType,
    ticker: isInvestment ? formState.ticker.trim() : null,
    shares: isInvestment ? Number(formState.shares) || null : null,
    pricePerShare: isInvestment ? Number(formState.pricePerShare) || null : null,
...
export function getRecurrenceSummary(sched, translate) {
  ...
  if (sched.recurrence_type === "day_of_week") {
    const days = (sched.days_of_week || []).map((d) => translate(WEEKDAY_KEYS[d])).join(", ");
